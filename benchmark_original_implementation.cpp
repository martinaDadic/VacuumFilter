...
string vrijemeLookupa(VacuumFilter<uint16_t, 16>& filter, string fileName,
                      int negativeQueries);
...
int main() {
  ...
  results_file
      << "data,sequence length,k,total k-mers,unique k-mers,positive "
         "queries,negative queries,number of buckets,insert time,number of "
         "failed inserts,lookup time,number of false negatives,false positive "
         "rate,remove time,number of failed removes,seed,memory consumption\n";
  while (getline(data_summary_file, line)) {
    ...
    noOfBuckets =
        (size_t)(stol(line.substr(0, line.find(';'))) / 3.8);  // 3.8 = 4*0.95
    line.erase(0, line.find(';') + 1);
    results_file << line.substr(0, line.find(';')) << ",";
    line.erase(0, line.find(';') + 1);
    ...
    results_file << vrijemeLookupa(filter, line.substr(0, line.find(';')),
                                   negativeQueries)
                 << ",";
    line.erase(0, line.find(';') + 1);
...
string vrijemeLookupa(VacuumFilter<uint16_t, 16>& filter, string fileName,
  ...
  return to_string(elapsed_time) + "," + to_string(brojFalseNegativa) + "," +
